  ...
  >
    <div class="flex items-start gap-6">
      <div class="flex flex-1 flex-col gap-3">
        <p
          v-if="courseTitle"
          class="text-2xl leading-tight font-bold tracking-[-0.02em] text-black"
        >
          Course: {{ courseTitle }}
        </p>
      ...
      <span
        class="shrink-0 rounded px-2 py-1 text-base font-medium"
        :class="badgeClasses[assignment.status]"
      >
    ...
    <template v-if="openInCourseHref">
      <template v-if="isEditable">
        <div class="flex items-end gap-6 rounded-lg border border-[#D6D9E4] bg-white p-7">
          <div class="flex flex-1 flex-col gap-1">
            ...
            <textarea
              ...
              rows="4"
              placeholder="Answer..."
              class="min-h-30 w-full resize-none rounded-lg border border-[#D6D9E4] p-3 text-base text-black outline-none placeholder:text-[#9AA1B9] focus:border-blue-500"
            ></textarea>
          </div>
          <div class="flex shrink-0 flex-col items-center gap-4">
            ...
            <RouterLink
              :to="openInCourseHref"
              class="text-base font-bold text-blue-600 transition-colors duration-200 hover:text-blue-700"
            >
              Open in Course
            </RouterLink>
          </div>
      ...
      <div v-else class="flex items-center gap-6 rounded-lg border border-[#D6D9E4] bg-white p-7">
        <div class="flex flex-1 flex-col gap-4">
          <p class="text-base text-black">{{ assignment.question }}</p>
          <p class="text-base text-[#646D89]">{{ assignment.answer }}</p>
        </div>
        <RouterLink
          :to="openInCourseHref"
          class="shrink-0 text-base font-bold text-blue-600 transition-colors duration-200 hover:text-blue-700"
        >
          Open in Course
        </RouterLink>
      </div>
