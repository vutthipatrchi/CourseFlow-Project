...
function buildCoursePayload(): CoursePayload {
  return {
    ...
    lessonItems: lessons.value.map((lesson) => ({
      // Only send server ids. Local draft ids must not UPDATE another course's lessons.
      ...(isEditing && lesson.id > 0 ? { id: lesson.id } : {}),
      name: lesson.name,
...
async function addLesson() {
  ...
  // Draft course: persist first so the lesson page has a real courseId for the API.
  if (!validateCourse()) {
    apiError.value = 'Please complete the course form before adding a lesson.'
    return
  }
  ...
  try {
    ...
    await router.push({
      name: 'admin-lesson-create',
      params: { courseId: String(created.id) },
    })
  } catch (error) {
...
function editLesson(lesson: CourseLesson) {
  const targetCourseId =
    isEditing && Number.isInteger(courseId) && courseId > 0
      ? String(courseId)
      : courseToEdit.value
        ? String(courseToEdit.value.id)
        : 'new'

  if (targetCourseId === 'new') {
    apiError.value = 'Please create the course before editing a lesson.'
    return
  }
...
function onLessonDragStart(index: number, event: DragEvent) {
...
function onLessonDragOver(index: number, event: DragEvent) {
  event.preventDefault()
  dragOverIndex.value = index
  if (event.dataTransfer) event.dataTransfer.dropEffect = 'move'
}

function onLessonDrop(index: number, event: DragEvent) {
  event.preventDefault()
      ...
      <form novalidate @submit.prevent="saveCourse">
        ...
        <main class="content">
          ...
          <section class="lessons-section">
            ...
            <div class="lessons-card">
              <div class="lesson-table">
                <div class="lesson-head">
                  <span class="lesson-drag-col" aria-hidden="true"></span>
                  <span></span>
                ...
                <div
                  ...
                  @dragover="onLessonDragOver(index, $event)"
                  @drop="onLessonDrop(index, $event)"
                ...
                >
                  <button
                    ...
                    @dragstart="onLessonDragStart(index, $event)"
            ...
            <div v-if="isEditing" class="lessons-footer">
              <button class="delete-course-link" type="button" @click="onDeleteCourse">
                Delete Course
              </button>
            </div>
          </section>
...
.lessons-card {
  ...
  border-radius: 12px;
  background: #fff;
  box-shadow: 0 4px 16px rgba(42, 46, 63, 0.04);
}
.lesson-table {
  width: 100%;
  background: #fff;
...
.lesson-row {
  display: grid;
  grid-template-columns: 48px 64px minmax(200px, 1fr) 140px 112px;
  align-items: center;
  width: 100%;
  padding: 0 32px;
  box-sizing: border-box;
...
.lesson-drag-handle svg {
  width: 10px;
  height: 16px;
  fill: currentColor;
  stroke: none;
}
.lesson-row .lesson-drag-handle:hover {
  background: transparent;
  color: #9aa1b9;
}
.lesson-row input {
